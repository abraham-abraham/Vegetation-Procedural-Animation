...
Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	for (i = 0; i < mesh->mNumVertices; i++) {
		...
		if (mesh->mMaterialIndex < scene->mNumMaterials)
		{
			...
			if (AI_SUCCESS == aiGetMaterialColor(mat, AI_MATKEY_COLOR_DIFFUSE, &diffuse)) {
				...
				vertex.color = glm::vec4(f[0], f[1], f[2], f[3]);
			}
			else {
	...
	cout << "Processing Bone Structures" << endl; 

	ExtractBoneWeightForVertices(vert, mesh, scene);

	for(Vertex v : vert) {
		float weightSum = 0;

		for (int weight : v.m_Weights) {
			weightSum += weight;
		}

		if(weightSum > 100){

			cout << "ERROR - Weight exceeds maximum per vertex!" << endl;
		}
	}
...
{
	cout << "Extracting bone weight for model vertices." << endl; 
	...
	for (int boneIndex = 0; boneIndex < mesh->mNumBones; ++boneIndex)
	{
		...
		assert(boneID != -1);
		auto weights = mesh->mBones[boneIndex]->mWeights; //Fetches the weights from the mesh itself
	...
	cout << "Bone weight extracted."<< endl;
...
unsigned int  Texture::loadFromFile(const char* path, string directory)
{
	...
	//string pathToLook = folders + name;

	string pathToLook = directory+ "/" + name;

	glGenTextures(1, &textID);
	//unsigned char* result = stbi_load((folders + name).c_str(), &width, &height, &num, 0);

	unsigned char* result = stbi_load((pathToLook).c_str(), &width, &height, &num, 0);
